...
const apiKey = "PyvP4KPAsMSXyaS5r4Ix5p3eKzArENccX72kkwYQ1O280xE1wK3PSyfNssLG3bJ0";

export const getContractNFTs = async (tokenAddress) => {
    const url = new URL(`${origin}/api/v2.2/nft/${tokenAddress}`);
  ...
  export const getContractTrades = async (tokenAddress) => {
    const url = new URL(`${origin}/api/v2.2/nft/${tokenAddress}/trades`);
    url.searchParams.append("chain", "eth");
  ...
  export const getNFTTransfers = async (tokenAddress, tokenId) => {
    const url = new URL(
      `${origin}/api/v2.2/nft/${tokenAddress}/${tokenId}/transfers`
    );
    url.searchParams.append("chain", "eth");
    url.searchParams.append("format", "decimal");
    url.searchParams.append("limit", "20");
